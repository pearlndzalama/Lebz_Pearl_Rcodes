...
#==================================================================================
# Extra High Spatial Autocorrelation Data Generation
#==================================================================================

# Data was simulated based on https://rpubs.com/jguelat/autocorr
...
Y <- (beta_0 + beta_1*values(Xvar3) + beta_2*values(Xvar3)^2 + 
           2*values(Xraster1) + 3*values((Xraster2))+ eps) # we need an error term attached to our y_variable
...
mtext("y = (0.1 + 0.5(Xvar3) - 0.5(Xvar3)^2 + 2(Xvar1) + 3(Xvar2) + E)", side = 3, line = -2, outer = TRUE)
...
mtext("Relationship between the covariates and y variable", side = 3, line = -2, outer = TRUE)
...
list_of_packages = c("sf", "raster", "mlr", "ranger", "RQGIS", "parallelMap")
...
task2 = mlr::makeRegrTask(data = rp, target = "y_variable", coordinates = co)

lrns = mlr::listLearners(task2, warn.missing.packages = FALSE)
dplyr::select(lrns, class, name, short.name, package)

lrn2 = mlr::makeLearner(cl = "regr.randomForest", predict.type = "response") # random forest model
lrn2$par.set #OR
getParamSet("regr.randomForest")
...
integer_ps = makeParamSet(
  makeIntegerParam("mtry", lower = 1, upper = 3),
  makeIntegerParam("sampsize", lower = 1, upper = 100)
)

print(integer_ps)

tune_level = mlr::makeResampleDesc(method = "SpCV", iters = 5)
ctrl = mlr::makeTuneControlRandom(maxit = 50) # random search with 50 iterations, to find the optimal hyperparameters 

wrapped_lrn_rf = 
  mlr::makeTuneWrapper(learner = lrn2,
                                 #inner loop
...
perf_level2 = mlr::makeResampleDesc(method = "SpRepCV",
                                    folds = 5,
                                    reps = 2) # change back to 100 


set.seed(44) # this might be slow, run it after doing time series 
cv_sp_rf = mlr::resample(learner = wrapped_lrn_rf,
                         task = task2,
                         resampling = perf_level2,
                         extract = getTuneResult,
                         measures = mlr::rmse)

cv_sp_rf$measures.test$rmse
cv_sp_rf$aggr
boxplot(cv_sp_rf$measures.test$rmse,
        col = "cornflowerblue",
        names = "Spatial Cross Validation (5-Fold)",
        ylab = "RMSE")# 10432.8788957

# normal (non-spatial) cross validation 

task_nsp = makeRegrTask(data = rp, target = "y_variable")
perf_level_nsp = makeResampleDesc(method = "RepCV", folds = 5, reps = 2) # change reps to 100

tune_level_nsp = mlr::makeResampleDesc(method = "CV", iters = 5)
ctrl_nsp = mlr::makeTuneControlRandom(maxit = 50) # random search with 50 iterations, to find the optimal hyperparameters 
...
set.seed(33)
cv_nsp = mlr::resample(learner = wrapped_lrn_rf_nsp,
                       task = task_nsp,
                       resampling = perf_level_nsp,
                       extract = getTuneResult,
                       measures = mlr::rmse)

cv_nsp$measures.test$rmse
cv_nsp$aggr
boxplot(cv_nsp$measures.test$rmse)

# normal cv vs spatial cv 

boxplot(cv_sp_rf$measures.test$rmse, cv_nsp$measures.test$rmse,
...
task_nsp_lm = mlr::makeRegrTask(data = rp, target = "y_variable")
perf_level_nsp_lm = mlr::makeResampleDesc(method = "RepCV", folds = 5, reps = 100)
cv_nsp_lm = mlr::resample(learner = lrn_lm,
                       task = task_nsp_lm,
                       resampling = perf_level_nsp_lm,
                       measures = mlr::rmse) # 1.2296707

boxplot(cv_sp_lm$measures.test$rmse, cv_nsp_lm$measures.test$rmse,
         col = c("cornflowerblue", "grey"),
         names = c("Spatial CV", "Conventional CV"),
         ylab = "RMSE")
...
wrapped_lrn_svm = makeTuneWrapper(learner = lrn_svm, 
                                  resampling = tune_level,
                                  par.set = ps,
                                  control = ctrl, 
                                  show.info = TRUE,
